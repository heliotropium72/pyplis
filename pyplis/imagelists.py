...
class ImgList(BaseImgList):
    ...
    **dilcorr_settings
    ...
    def __init__(self, files=[], list_id=None, list_type=None, camera=None,
        ...
        self._bg_img = None #a constant background image that may be assigned to the list using attr. bg_img
...
# =============================================================================
    ...
    def sensitivity_corr_mode(self, val):
        ...
        if val == self._list_modes["senscorr"]:
            return
        if val:    
            self.senscorr_mask #raises AttributeError if mask is not available
            if not self.aa_mode:
        ...
        if not isinstance(self._senscorr_mask, Img):
            try:
                if not isinstance(self.calib_data, CalibData):
                    raise AttributeError("No CalibData object assigned to list "
                                         "from which a sensitivity correction "
                                         "mask might be loaded...")
...
# =============================================================================
    ...
    def correct_dilution(self, img, plume_bg_vigncorr=None, 
        ...
        # get current extinction coefficient, raises AttributeError if not 
        # available
        ext_coeff = float(ext_coeff) if ext_coeff is not None else self.ext_coeff
        # initiate variable that may be filled with the uncorrected plume background
        # (if this is an onband list in AA mode, see below)
        if calc_mask or compute_bg:
            ...
            tau_uncorr = self.bg_model.get_tau_image(img, bg_raw,
                                                     check_state=
                                                     img_check_plumemask)
    ...
    def _apply_edit(self, key):
        ...
        if self.dilcorr_mode:
            ...
            if self.aa_mode:
                ...
                # if this is an onband list that is in AA mode, the plume pixel mask 
                # is retrieved from a precomputed AA image which is done below. This 
                # allows in one go to also retrieve the actual plume background in 
                # the offband from the already computed onband background 
                off_list = self.get_off_list()
                off_img = off_list.loaded_images[key].to_pyrlevel(img.pyrlevel)
                if off_img.is_vigncorr:
                    warn("List %s is in AA mode: it appears that vigncorr_mode"
                         " is active in either or both of the lists %s and %s. "
                         "You might want to consider to turn off vigncorr_mode "
                         "as this is irrelevant for the computation of OD and "
                         "AA images and only slows things down a little")
                ...
                aa_uncorr = self.bg_model.get_aa_image(img, off_img, 
                                                       bg_on, bg_off)
                plume_pix_mask = self.calc_plumepix_mask(aa_uncorr, 
                                                         s.tau_thresh, 
            ...
            (img, # vignetting corrected and dilution corrected image
             bg_vigncorr, 
             plume_pix_mask) = self.correct_dilution(img,
                                                     plume_pix_mask=
                                                     plume_pix_mask,
            ...
            if self.tau_mode:
                ...
                (off_img, 
                 _, 
                 _) = off_list.correct_dilution(off_img,
                                                plume_bg_vigncorr=
                                                bg_off_vigncorr,
                                                plume_pix_mask=plume_pix_mask)
